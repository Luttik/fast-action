...
The mock must match `src/FastAction/Overlay/OverlayWindow.xaml` and the layout/appearance model:
...
- Settings include matrix size, start-key picker, theme/tiles/corners, **acrylic + opacity**, and the **Lucide color palette**.
...
Do not commit mock-only experiments into `docs/screenshots/`. Replace the four named files in place so git diffs show visual changes.

## Checklist after overlay UI work

- [ ] Mock updated to match XAML (menu, tiles, settings, acrylic, Lucide swatches)
